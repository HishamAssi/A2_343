...
FROM 
	...
	WHERE country.name = 'Japan') AS election_cabinets 
...
FROM election JOIN cabinet ON election.country_id = cabinet.country_id JOIN country ON election.country_id = country.id AND country.name='Japan'
WHERE (
election.e_date IN 
